...
### 🎯 Mục tiêu giải quyết:
- **Kiểm soát tính toàn vẹn (Invariant Integrity):** Đảm bảo 100% không bao giờ xảy ra lỗi "thiếu đạn" hoặc "thừa đạn" giữa ma trận điểm ảnh (`blockData`) và tổng số phát bắn của xe (`shooterData`), tự động cảnh báo type màu rác ngoài palette `[0..35]`.
...
- **Tạo sinh Level theo Ý đồ thiết kế (Design Intent Generator):** Tự động tái tạo cấu hình dàn xe súng và bãi đỗ theo 10 hồ sơ ý đồ chiến thuật (Rush, Squeeze, Freeze, Bomb...) mà vẫn **bảo toàn tuyệt đối 100% hình vẽ Pixel Art gốc**.
- **Playtest giả lập tức thời (Zero Build Playtest):** Chơi thử ngay trên trình duyệt mà không cần mở Unity Editor hay build APK/IPA.
- **Tối ưu hóa bằng dữ liệu BI (Data-Driven Level Tuning):** Nhập dữ liệu Analytics từ Firebase/Amplitude để tìm ra ngay các level có tỷ lệ thua bất thường, drop-off cao, từ đó tinh chỉnh độ khó kịp thời.
